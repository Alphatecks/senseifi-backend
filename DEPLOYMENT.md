...
**On-demand sync:** `POST /api/dashboard/:address/assets/sync` — uses Moralis `chain` as **hex** (e.g. `0x38` for BSC). Paginates with `cursor` until exhausted. Respect Moralis rate limits; add a cron or debounce on the client if needed.
