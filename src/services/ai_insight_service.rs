...
impl AiInsightService {
    ...
    pub fn explain_risks(
        ...
        if simulation.hidden_internal_calls.unwrap_or(0) > 0 {
            parts.push(
                "It uses hidden internal calls that can change your allowances or move funds."
                    .to_string(),
            );
        }
        ...
        let privileged = owner_privileges.mint == Some(true)
            || owner_privileges.withdraw_liquidity == Some(true);
        if privileged
            && (simulation.drains_full_balance == Some(true)
